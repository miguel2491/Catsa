...
import {getPedidoInd} from '../../Utilidades/Funciones'
...
const RBPedido = () => {
  ...
  const getRepos = async () =>{
    ...
    try {
      ...
      console.log(data[0]);
      if (data) {
        setDataPedido(data[0]); // Aquí se asigna el primer objeto del array (suponiendo que es el único)
      } else {
  ...
  useEffect(()=>{    
    if(fetchedData)
    {
      // Asignar los valores a las constantes del estado
      setPlanta(fetchedData.Planta);
      setAsesor(fetchedData.asesor);
      setProgramado(fetchedData.activo);
      setVBueno(fetchedData.VistoBueno);
      setObra(fetchedData.NoObra);
      setDireccion(fetchedData.direccion);
      setProducto(fetchedData.Producto);
      setM3(fetchedData.M3Viaje);
      setBomba(fetchedData.CodBomba);
      setFcaEntrega(fetchedData.FechaHoraPedido);
      setHraSalida(fetchedData.hraSalida);
      setRecorrido(fetchedData.TRecorrido);
      setRAprox(fetchedData.TiempoReal);
      setEspaciado(fetchedData.Espaciado);
      setEstatus(fetchedData.estatus);
      setPProducto(fetchedData.PrecioProducto);
      setTotalPagar(fetchedData.iTotalPagar);
                ...
                <CCard className="mb-4">
                    ...
                    <CCardBody>
                        ...
                        <CRow>
                          ...
                          <CCol xs={6} md={3}>
                            <CFormInput type='text' id='m3' label='M3' disabled value={m3}/>
                          </CCol>
                        </CRow>
                        <CRow>
                          <CCol xs={6} md={3}>
