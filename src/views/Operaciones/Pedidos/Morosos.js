...
import { cilCheck, cilEnvelopeLetter, cilSearch, cilSync, cilTrash, cilX } from '@coreui/icons';
import {getPedidoMoroso, setRecuperarMoroso, sAvisoMoroso} from '../../../Utilidades/Funciones'
...
const PMorosos = () => {
    ...
    const getPedidoMoroso_ = async () =>{
        const auxFcaI = format(vFechaI, 'yyyy/MM/dd');
        const auxFcaF = format(vFcaF, 'yyyy/MM/dd');
        try {
            const data = await getPedidoMoroso(auxFcaI, auxFcaF);
    ...
    const sendAviso = async () =>{
        ...
        try {
            const result = await sAvisoMoroso(dtPE);
      ...
      <CContainer fluid>
        ...
        <CRow>
            ...
            <CCol xs={12} md={2} style={{'margin-top':'30px'}}>
                <CButton color='primary' onClick={getPedidoMoroso_}>
